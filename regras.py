...
if __name__ == '__main__':
    while True:
        ...
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda <= 556.09497 and prec <= 1034.39551:
            print "terminalNode = -1 \nmean = 0.0835317"
        #Rules for terminal node 2
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda <= 556.09497 and prec > 1034.39551 and prec <= 1090.74597:
            print "terminalNode = -2 \nmean = 0.330305"
        #Rules for terminal node 3
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda <= 556.09497 and prec > 1090.74597 and prec <= 1109.07446:
            print "terminalNode = -3 \nmean = 0.18456"
        #Rules for terminal node 4
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda > 556.09497 and renda <= 841.71002 and prec <= 1109.07446:
            print "terminalNode = -4 \nmean = 0.110923"
        #Rules for terminal node 5
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda > 841.71002 and renda <= 864.86499 and prec <= 1109.07446:
            print "terminalNode = -5 \nmean = 0.434419"
        #Rules for terminal node 6
        if uso_terra <= 8 and dens_demo <= 3.63463 and renda > 864.86499 and prec <= 1109.07446:
            print "terminalNode = -6 \nmean = 0.175612"
        #Rules for terminal node 7
        if uso_terra <= 8 and dens_demo <= 3.26368 and prec > 1109.07446 and VCF <= 54.5:
            print "terminalNode = -7 \nmean = 0.109868"
        #Rules for terminal node 8
        if uso_terra <= 8 and dens_demo <= 3.26368 and prec > 1109.07446 and VCF > 54.5 and DEM <= 44.50000 and TEMP <= 24.79185:
            print "terminalNode = -8 \n mean = 0.0168376"
        #Rules for terminal node 9
        if uso_terra <= 8 and dens_demo <= 3.26368 and prec > 1109.07446 and VCF > 54.5 and DEM <= 44.50000 and TEMP > 24.79185:
            print "terminalNode = -9 \nmean = 0.131192"
        #Rules for terminal node 10
        if uso_terra <= 8 and dens_demo <= 3.26368 and prec > 1109.07446 and VCF > 54.5 and DEM > 44.50000:
            print "terminalNode = -10 \nmean = 0.0499697"
        #Rules for terminal node 11
        if uso_terra <= 8 and dens_demo > 3.26368 and dens_demo <= 3.63463 and prec > 1109.07446:
            print "terminalNode = -11 \nmean = 0.139411"
        #Rules for terminal node 12
        if uso_terra > 8 and dens_demo <= 3.63463 and renda <= 859.72498 and prec <= 1350.50757 and temp <= 24.75305:
            print "terminalNode = -12 \nmean = 0.0372407"
        #Rules for terminal node 13
        if uso_terra > 8 and dens_demo <= 3.63463 and renda <= 859.72498 and prec <= 1350.50757 and temp > 24.75305:
            print "terminalNode = -13 \nmean = 0.150805"
        #Rules for terminal node 14
        if uso_terra > 8 and dens_demo <= 3.63463 and renda > 859.72498 and prec <= 1350.50757:
            print "terminalNode = -14 \nmean = 0.250526"
        #Rules for terminal node 15
        if uso_terra > 8 and dens_demo <= 3.63463 and renda <= 684.70502 and prec > 1350.50757 and temp <= 24.68275:
            print "terminalNode = -15 \nmean = 0.281141"
        #Rules for terminal node 16
        if uso_terra > 8 and dens_demo <= 3.63463 and renda > 684.70502 and renda <= 854.57001 and prec > 1350.50757 and temp <= 24.68275:
            print "terminalNode = -16 \nmean = 0.748276"
        #Rules for terminal node 17
        if uso_terra > 8 and dens_demo <= 2.35704 and renda > 854.57001 and prec > 1350.50757 and temp <= 24.68275:
            print "terminalNode = -17 \nmean = 0.0606344"
        #Rules for terminal node 18
        if uso_terra > 8 and dens_demo > 2.35704 and dens_demo <= 3.63463 and renda > 854.57001 and prec > 1350.50757 and temp <= 24.68275:
            print "terminalNode = -18 \nmean = 0.52072"
        #Rules for terminal node 19
        if uso_terra > 8 and dens_demo <= 2.35704 and prec > 1350.50757 and temp > 24.68275 and temp <= 24.71705:
            print "terminalNode = -19 \nmean = 0.0901171"
        #Rules for terminal node 20
        if uso_terra > 8 and dens_demo <= 2.35704 and prec > 1350.50757 and temp > 24.71705:
            print "terminalNode = -20 \nmean = 0.338749"
        #Rules for terminal node 21
        if uso_terra > 8 and dens_demo > 2.35704 and dens_demo <= 2.50642 and prec > 1350.50757 and temp > 24.68275:
        ...
        if (uso_terra <=4 or uso_terra == 6 or uso_terra == 8) and dens_demo > 3.63463 and prec > 1169.50452 and renda <= 736.80499 and DEM <= 55.50000:
            print "terminalNode = -25 \nmean = 0.0639006"
        #Rules for terminal node 26
        if (uso_terra == 5 or uso_terra == 7 or uso_terra > 8) and dens_demo > 3.63463 and prec > 1169.50452 and prec <= 1371.60449 and renda <= 736.80499 and DEM <= 55.50000:
            print "terminalNode = -26 \nmean = 0.125505"
        #Rules for terminal node 27
        if (uso_terra == 5 or uso_terra == 7 or uso_terra > 8) and dens_demo > 3.63463 and dens_demo <= 24.23654 and prec > 1371.60449 and renda <= 736.80499 and DEM <= 55.50000:
            print "terminalNode = -27 \nmean = 0.0750805"
        #Rules for terminal node 28
        if (uso_terra == 5 or uso_terra == 7 or uso_terra > 8) and dens_demo > 24.23654 and prec > 1371.60449 and renda <= 736.80499 and DEM <= 55.50000:
